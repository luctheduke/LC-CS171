...
MORE_EVENTS:
  ...
  // printf("at least one dimension of new win is larger than pixmaxp\n") ;
    ...
    *px = 0 ; *py = 0 ;
    return -2000 ; // This actually does happen
...
int Get_Image_From_File_X (void *filename, int x, int y)
...
int Get_Pixel_SAFE_X (int x, int y, int pixel[1]) 
// return 1 if successful, else 0
{
  XImage *pxim ;
...
int XImage_to_Display (XImage *pxim, int x, int y)
// Put lower left corner of file into the graphics window at (x,y).
// return 1 if successful else 0
{
  int image_width, image_height ;
