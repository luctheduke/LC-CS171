...
int Init_X (int swidth, int sheight)
{
    ...
    if (swidth > maxwidth) {
      ...
      printf("Display window width truncated.\n") ;
    ...
    if (sheight > maxheight) {
      ...
      printf("Display window height truncated.\n") ;
    ...
    ThePixmap = XCreatePixmap(TheDisplay, TheRootWindow, The_Pix_width, The_Pix_height, 
				TheDepth);
...
MORE_EVENTS:
	...
	*px = 0 ; *py = 0 ;
	...
	if((event.xconfigure.width == The_Win_width) &&
	  (event.xconfigure.height == The_Win_height)){ break; }

	// display window resize
	The_Win_width = event.xconfigure.width;
	The_Win_height = event.xconfigure.height;

	// new if stmnt
        if ((The_Win_width > The_Pix_width)||
            (The_Win_height > The_Pix_height)) {

	  XFreePixmap(TheDisplay, ThePixmap);

          if (The_Win_width > The_Pix_width) The_Pix_width = The_Win_width ;
          if (The_Win_height > The_Pix_height) The_Pix_height = The_Win_height ;
